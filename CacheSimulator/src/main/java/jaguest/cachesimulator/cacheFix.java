...
public class cacheFix {
     ...
     */
    public cacheFix(int associativity,int blockSize,int cacheSize){
        ...
        for(int i = 0;i < this.sets.length;i++){ // iterate over # of sets
            LRU.put(i,new LinkedList<Integer>()); // populate the hashtable with lists associated with a key for each set number
        }
        ...
        for(int i = 0;i < sets.length;i++){
            for(int j = 0;j < sets[i].length;j++){
                ...
                this.LRU.get(i).addLast(j); // will add the block number to spot for most recently used
                // now both the caceh and the LRU hashtable should be fully populated with initial INVL values after nested loop ends
            }
        }
